import {
  ...
  describe('selectUtxos', function() {
    ...
    const case1 = selectUtxos(utxos1, 10000000);
    it('correct format', function() {
      expect(case1.total).toEqual(jasmine.any(Number));
      expect(case1.utxos).toEqual(jasmine.any(Array));
      case1.utxos.forEach( item => {
        expect(item.address).toEqual(jasmine.any(String));
        expect(item.satoshis).toEqual(jasmine.any(Number));
        expect(item.script).toEqual(jasmine.any(String));
        expect(item.txid).toEqual(jasmine.any(String));
        expect(item.outputIndex).toEqual(jasmine.any(Number));
      });
    });
    it('correct value', function() {
      expect(case1.total).toBe(90900111);
      expect(case1.utxos[0].address).toBe('XxVj9BqtZNZTwEeE9MdWF1prgc8SKVpZ7d');
      expect(case1.utxos[0].satoshis).toBe(90900111);
      expect(case1.utxos[0].script).toBe('76a914ef389b41f17223741592c9880682240f2774caa588ac');
      expect(case1.utxos[0].txid).toBe('483bdb594fe347052cb56c60000fba593ef7dee8bf3f069e616fbdf8353e38ae');
    ...
    const case3 = selectUtxos(utxos3, 1000);
    it('correct value', function() {
      expect(case3.total).toBe(1050);
      expect(case3.utxos.length).toBe(3);
    });
  });
