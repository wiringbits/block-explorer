  ...
  "getBlockRewards" should {
    ...
    "get correct rewards for block 1663808" in {
      ...
      service.getBlockRewards(block, BlockExtractionMethod.ProofOfStake).map { rewards =>
        rewards.get match {
          case PoSBlockRewards(coinstake, Some(masternode), None, stakedAmount, stakedDuration) =>
    ...
    "get correct rewards for block 1671552" in {
      ...
      service.getBlockRewards(block, BlockExtractionMethod.TrustlessProofOfStake).map { rewards =>
        rewards.get match {
          case TPoSBlockRewards(owner, merchant, Some(masternode), None, stakedAmount, stakedDuration) =>
    ...
    "get correct rewards for block 898488" in {
      val blockHash = "c6c9a39dfe3abc01db2a6049133d9770349dd0a5627e8446b698b3d96c63dc84"
      val block = BlockLoader.getFullRPC(blockHash)
      ...
      service.getBlockRewards(block, BlockExtractionMethod.TrustlessProofOfStake).map { rewards =>
        rewards.get match {
          case TPoSBlockRewards(owner, merchant, None, Some(treasury), stakedAmount, stakedDuration) =>
    ...
    "get correct rewards for block 129600" in {
      val blockHash = "76848793c90ee23b7de68c1c97d0a6dd8d53921c48854fac53a22e56c8549b18"
      val block = BlockLoader.getFullRPC(blockHash)
      ...
      service.getBlockRewards(block, BlockExtractionMethod.ProofOfStake).map { rewards =>
        rewards.get match {
          case PoSBlockRewards(coinstake, None, Some(treasury), stakedAmount, stakedDuration) =>
