...
object CurrencySynchronizerConfig {
  ...
  def apply(config: Configuration): CurrencySynchronizerConfig = {
    ...
    val initialDelay: FiniteDuration =
      config.get[FiniteDuration]("currencySynchronizer.initialDelay")
    val highPriorityInterval: FiniteDuration =
      config.get[FiniteDuration]("currencySynchronizer.highPriorityInterval")
    val lowPriorityInterval: FiniteDuration =
      config.get[FiniteDuration]("currencySynchronizer.lowPriorityInterval")

    CurrencySynchronizerConfig(
      enabled,
      initialDelay,
