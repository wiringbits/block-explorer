...
case class NodeStatsSynchronizerConfig(
    enabled: Boolean,
...
object NodeStatsSynchronizerConfig {
  ...
  def apply(config: Configuration): NodeStatsSynchronizerConfig = {
    ...
    val initialDelay: FiniteDuration =
      config.get[FiniteDuration]("nodeStatsSynchronizer.initialDelay")
    val interval: FiniteDuration =
      config.get[FiniteDuration]("nodeStatsSynchronizer.interval")
