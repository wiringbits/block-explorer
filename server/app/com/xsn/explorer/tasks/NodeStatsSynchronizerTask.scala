      ...
      actorSystem.scheduler.schedule(config.initialDelay, config.interval) {
        statisticsService.getCoinsStaking().onComplete {
          case Success(Good(coinsStaking)) =>
            ...
            nodeStatsSynchronizerActor.ref ! NodeStatsSynchronizerActor.UpdateCoinsStaking(coinsStaking)
          case Success(Bad(error)) =>
            logger.warn(s"CoinsStaking information syncronization failed due to $error")
          case Failure(exception) =>
            logger.error(s"CoinsStaking information syncronization failed due to ${exception.getMessage}", exception)
        }
      }
    } else {
