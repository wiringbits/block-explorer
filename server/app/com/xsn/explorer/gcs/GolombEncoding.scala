...
class GolombEncoding(p: Int, m: Int, key: SipHashKey) {
  ...
  /** Recovers the hashes from the encoded bytes.
    *
    * This method doesn't handle corrupted inputs, which shouldn't be a problem because the method is used only to
    * verify that the filter is correct.
    *
    * @param encoded
    *   the encoded bytes, we expect them to be correct
    * @param n
    *   the number of words encoded in the bytes
    * @return
    *   the recovered sorted set of hashes
    */
  ...
  /** NOTE: This is a copy from https://github.com/btcsuite/btcutil/blob/master/gcs/gcs.go that is used for
    * compatibility reasons, here we don't care about such optimizations because a filter is built once per block and
    * never queried.
    *
    * Original docs: fastReduction calculates a mapping that's more ore less equivalent to: x mod N.
    *
    * However, instead of using a mod operation, which using a non-power of two will lead to slowness on many processors
    * due to unnecessary division, we instead use a "multiply-and-shift" trick which eliminates all divisions, described
    ...
    * we'll be super portable and unfold the operation into several operations with 64-bit arithmetic. As inputs, we the
    * number to reduce, and our modulus N divided into its high 32-bits and lower 32-bits.
    */
