...
private[synchronizer] class LedgerSynchronizationStatusService @Inject()(
  ...
  def getSyncingStatus(candidate: rpc.Block): FutureApplicationResult[SynchronizationStatus] = {
    syncOps.getLatestLedgerBlock.toFutureOr.flatMap {
      case Some(latestLedgerBlock) =>
        for {
          ...
          if (lca.blockhash == candidate.hash) {
            ...
            SynchronizationStatus.MissingBlockInterval(latestLedgerBlock.height.int to candidate.height.int)
          } else {
      ...
      case None =>
        val result = SynchronizationStatus.MissingBlockInterval(0 to candidate.height.int)
  ...
  private def findNewestStoredBlockFromChain(candidate: rpc.Block): FutureOr[BlockPointer] = {
    blockDataHandler
      ...
      .flatMap {
        case Some(_) =>
          Future.successful(Good(BlockPointer(candidate.hash, candidate.height))).toFutureOr

        case None =>
          candidate.previousBlockhash match {
            case Some(previous) =>
              xsnService.getBlock(previous).toFutureOr.flatMap(findNewestStoredBlockFromChain)
            case None =>
  ...
  private def findNewestChainBlockFromStorage(block: persisted.Block): FutureOr[BlockPointer] = {
    xsnService
      ...
      .flatMap {
        case Some(blockhash) if blockhash == block.hash =>
          Future.successful(Good(BlockPointer(block.hash, block.height))).toFutureOr

        case _ =>
          block.previousBlockhash match {
            case Some(previous) =>
              blockDataHandler.getBy(previous).toFutureOr.flatMap(findNewestChainBlockFromStorage)
            case None =>
              Future.successful(Bad(BlockNotFoundError).accumulating).toFutureOr
          }
      }
