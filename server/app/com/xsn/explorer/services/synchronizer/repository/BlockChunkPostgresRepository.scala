...
class BlockChunkPostgresRepository @Inject()(
  ...
  override def upsertSyncState(blockhash: Blockhash, state: BlockSynchronizationState): ApplicationResult[Unit] = {
  ...
  override def findSyncState(blockhash: Blockhash): ApplicationResult[Option[BlockSynchronizationState]] = {
    withConnection { implicit conn =>
  ...
  override def findSyncingBlock(): ApplicationResult[Option[Blockhash]] = withConnection { implicit conn =>
    val result = synchronizationProgressDAO.findAny
    Good(result)
  }

  override def upsertBlock(block: Block): ApplicationResult[Unit] = withConnection { implicit conn =>
    blockDAO.upsert(block)
    Good(())
  }

  override def upsertFilter(blockhash: Blockhash, filter: GolombCodedSet): ApplicationResult[Unit] = {
    withConnection { implicit conn =>
  ...
  override def setNextBlockhash(blockhash: Blockhash, nextBlockhash: Blockhash): ApplicationResult[Unit] = {
    withConnection { implicit conn =>
  ...
  override def upsertTransaction(index: Int, transaction: Transaction): ApplicationResult[Unit] = {
    withConnection { implicit conn =>
      val _ = transactionDAO
        .upsertTransaction(index, transaction)
        .getOrElse(throw new RuntimeException(s"Failed to insert transaction ${transaction.id}"))
      Good(())
    }
  }

  override def upsertInput(txid: TransactionId, input: Transaction.Input): ApplicationResult[Unit] = {
    withConnection { implicit conn =>
  ...
  override def upsertOutput(output: Transaction.Output): ApplicationResult[Unit] = withConnection { implicit conn =>
    transactionOutputDAO.upsert(output)
    Good(())
  }

  override def spendOutput(txid: TransactionId, index: Int, spentOn: TransactionId): ApplicationResult[Unit] = {
    withConnection { implicit conn =>
  ...
  override def upsertAddressTransactionDetails(details: AddressTransactionDetails): ApplicationResult[Unit] = {
    withConnection { implicit conn =>
  ...
  override def closeContract(id: TPoSContract.Id, closedOn: TransactionId): ApplicationResult[Unit] = {
    withConnection { implicit conn =>
  ...
  override def upsertContract(contract: TPoSContract): ApplicationResult[Unit] = withConnection { implicit conn =>
   ...
   */
  override def atomicUpdateBalances(blockhash: Blockhash, balances: List[Balance]): ApplicationResult[Unit] = {
    withTransaction { implicit conn =>
      // the balances
      for (balance <- balances) {
        balanceDAO
          .upsert(balance)
          .getOrElse(throw new RuntimeException(s"Failed to upsert balance for ${balance.address}"))
      }
  ...
  override def atomicRollback(blockhash: Blockhash): ApplicationResult[Unit] = withTransaction { implicit conn =>
    // transactions
