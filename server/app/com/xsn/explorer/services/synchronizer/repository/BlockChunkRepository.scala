...
trait BlockChunkRepository[F[_]] {
  ...
  def upsertAddressTransactionDetails(
...
object BlockChunkRepository {
  ...
  class FutureImpl @Inject() (blocking: Blocking)(implicit ec: ExecutionContext)
    ...
    override def findSyncState(
        blockhash: Blockhash
    ): FutureApplicationResult[Option[BlockSynchronizationState]] = {
      Future {
    ...
    override def upsertBlock(block: Block): FutureApplicationResult[Unit] =
      Future {
    ...
    override def upsertContract(
        contract: TPoSContract
    ): FutureApplicationResult[Unit] = Future {
