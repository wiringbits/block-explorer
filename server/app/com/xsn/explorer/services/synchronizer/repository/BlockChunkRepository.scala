...
trait BlockChunkRepository[F[_]] {
  ...
  def upsertBlockReward(blockhash: Blockhash, reward: BlockReward): F[Unit]
}
...
object BlockChunkRepository {
  ...
  class FutureImpl @Inject()(blocking: Blocking)(implicit ec: ExecutionContext)
    ...
    override def upsertBlockReward(blockhash: Blockhash, reward: BlockReward): FutureApplicationResult[Unit] = Future {
