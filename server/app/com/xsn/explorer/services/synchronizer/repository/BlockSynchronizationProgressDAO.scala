...
class BlockSynchronizationProgressDAO {
  ...
  def upsert(blockhash: Blockhash, state: BlockSynchronizationState)(implicit
      conn: Connection
  ): Unit = {
    val _ = SQL(
      """
      ...
      """.stripMargin
    ).on("blockhash" -> blockhash.toBytesBE.toArray, "state" -> state.entryName)
      .execute()
  }

  def find(
      blockhash: Blockhash
  )(implicit conn: Connection): Option[BlockSynchronizationState] = {
    val maybe = SQL(
      """
        |SELECT state
      ...
      """.stripMargin
    ).on("blockhash" -> blockhash.toBytesBE.toArray)
