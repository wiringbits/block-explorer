...
object BlockRewardPostgresSerializer {
  case class Reward(blockReward: BlockReward, rewardType: RewardType, stake: Option[Stake])
  ...
  def serialize(reward: BlockRewards): List[Reward] = {
    reward match {
      ...
      case r: PoSBlockRewards =>
        val rewards = List(Reward(r.coinstake, RewardType.PoS, Some(Stake(r.stakedAmount, r.stakedDuration))))

        rewards ++ r.masternode.map(Reward(_, RewardType.Masternode, None))
      case r: TPoSBlockRewards =>
        val rewards = List(
          Reward(r.owner, RewardType.TPoSOwner, Some(Stake(r.stakedAmount, r.stakedDuration))),
  ...
  def deserialize(rewards: List[Reward]): Option[BlockRewards] = {
    rewards match {
      ...
      case Reward(r, RewardType.PoS, Some(Stake(stakedAmount, stakedTime))) :: Nil =>
      ...
      case Reward(owner, RewardType.TPoSOwner, Some(Stake(stakedAmount, stakedTime)))
      ...
      case Reward(masternode, RewardType.Masternode, _)
            :: Reward(owner, RewardType.TPoSOwner, Some(Stake(stakedAmount, stakedTime)))
            :: Reward(merchant, RewardType.TPoSMerchant, _) :: Nil =>
