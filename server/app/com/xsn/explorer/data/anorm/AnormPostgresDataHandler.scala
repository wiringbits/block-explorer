...
trait AnormPostgresDataHandler {
  ...
  def withConnection[A](
      block: Connection => ApplicationResult[A]
  ): ApplicationResult[A] = {
    try {
      ...
      case e: PSQLException if isIntegrityConstraintViolationError(e) =>
        val error =
          createForeignKeyViolationError(e).getOrElse(UnknownPostgresError(e))
  ...
  private def isIntegrityConstraintViolationError(e: PSQLException) =
    e.getSQLState startsWith "23"
  private def createForeignKeyViolationError(
      e: PSQLException
