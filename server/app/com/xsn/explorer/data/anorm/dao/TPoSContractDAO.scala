...
class TPoSContractDAO {
  ...
  )(implicit conn: Connection): TPoSContract = {
    SQL(
      """
  ...
  def deleteBy(
      txid: TransactionId
  )(implicit conn: Connection): Option[TPoSContract] = {
    SQL(
      """
  ...
  def close(id: TPoSContract.Id, closedOn: TransactionId)(implicit
      conn: Connection
  ): Unit = {
    val _ = SQL(
      """
  ...
  )(implicit conn: Connection): List[Address] = {
    SQL(
      """
        |SELECT owner
        |FROM tpos_contracts
